...
#ifdef DEBUG
  Serial.println("i: rawbuf[i] : rawbuf[i]*USECPERTICK");
  for(int counterr=0;counterr<results->rawlen;counterr++){
	Serial.print(counterr);
	Serial.print("     ");
	Serial.print(results->rawbuf[counterr]);
	Serial.print("     ");
	Serial.println(results->rawbuf[counterr]*USECPERTICK);
	}
	Serial.println("---------------");
#endif	
